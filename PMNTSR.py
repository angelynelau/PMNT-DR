...
for team in teams:
    ...
    if selected_fittings:
        selected_size = st.multiselect("Select size:", fittings[selected_fittings], key=f"fittingssize_{team}")
        if selected_size:
            number_input = st.number_input(min_value=0, step=1, key=f"fittingsnos_{team}")
