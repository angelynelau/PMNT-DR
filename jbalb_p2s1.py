...
# Total Working Hours
working_hours = st.number_input("Total Working Hours:", min_value=0, step=1)

# Time Working (e.g., 0800-1700)
working_time = st.text_input("Working Time:", "0800-1700")

# Machinery
excavator = st.number_input("Excavator -", min_value=0, step=1)
piling_rig = st.number_input("Piling Rig -", min_value=0, step=1)
crane = st.number_input("Crane -", min_value=0, step=1)
...
start_chainage = st.text_input("Starting Chainage (e.g., 400)")
end_chainage = st.text_input("Ending Chainage (e.g., 500)")
...
if st.button("Generate Report"):
    ...
    output += f"Total Working Hours: {working_hours} hrs\n"
    output += f"{working_time}\n"
    
    output += "Machinery\n"
    output += f"Excavator - {excavator}\n"
    output += f"Piling Rig - {piling_rig}\n"
    output += f"Crane - {crane}\n"
    ...
    if "Pipe Laying" in activity:
        ...
        if start_chainage and end_chainage:
            output += f"Pipe laying works from {start_chainage} to {end_chainage}\n"
